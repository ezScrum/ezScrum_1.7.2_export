...
public class MySQLQuerySet implements IQueryValueSet {	
	...
	*/
	...
	public String getSelectQuery() {
		...
		if (this._leftJoinList.size() != 0)
			query.append(" LEFT JOIN" + getLeftJoins());
		if (this._conditionList.size() != 0)
			query.append(" WHERE " + getConditions());
		if (!this._orderBy.equals(""))
			query.append(" ORDER BY `"+this._orderBy+"` "+ this._orderDirection);
